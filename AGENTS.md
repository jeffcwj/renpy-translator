...
### 2026-03-12: 双花括号还原逻辑重写（基于正则按位置替换）

#### 文件变更

- `src/string_tool.py` — 重写 `sanitize_translated_text` step 3 双花括号还原逻辑

#### 修复详情

- **问题根因**：原来的 `str.replace(broken, double, 1)` 方案有两个缺陷：
  1. 同名标签出现多次时（如两个 `{{/color}}`），`double in text` 检查会因第一个正确的就跳过所有残缺的
  2. `replace` 会匹配到刚修复过的内容（如 `{b}` → `{{b}}` 后，下一轮的 `{b}` 匹配到 `{{b}}` 中的子串），导致 `{{{{b}}}}`
- **新方案**：基于正则 lookbehind/lookahead 按位置精确匹配
  - `(?<!\{)\{tag\}\}\}` — 匹配 `{tag}}` 但不匹配 `{{tag}}`
  - `\{\{tag\}(?!\})` — 匹配 `{{tag}` 但不匹配 `{{tag}}`
  - `(?<!\{)\{tag\}(?!\})` — 匹配 `{tag}` 但不匹配 `{{tag}}` 和 `{tag}}`
- **计数机制**：统计原文中每个标签出现次数，与 text 中已正确的数量对比，只修复差额部分
- **`re.escape(tag)`**：标签内容可能含正则特殊字符（如 `color=[x]`），需转义
- 测试覆盖 11 个 Case 全部通过：单侧丢失、双侧丢失、重复标签、混合正确/残缺、特殊字符标签等
